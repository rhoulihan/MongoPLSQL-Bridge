...
/**
 * Represents a $group stage that groups documents by a specified _id expression.
 * Translates to Oracle's GROUP BY clause with aggregate functions.
 */
public final class GroupStage implements Stage {
    ...
    public GroupStage(Expression idExpression, Map<String, AccumulatorExpression> accumulators) {
        ...
        this.accumulators = accumulators != null
            ? new LinkedHashMap<>(accumulators)
            : new LinkedHashMap<>();
    }
