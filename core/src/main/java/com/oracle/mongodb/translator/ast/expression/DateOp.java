...
/**
 * Date operators for expressions. Maps MongoDB date operators to their Oracle SQL equivalents.
 *
 * <p>MongoDB stores dates as ISODate which is parsed from JSON as a string. Oracle needs
 * TO_TIMESTAMP to convert, then EXTRACT or TO_CHAR for components.
 */
public enum DateOp {
  ...
  /**
   * Returns true if this operator uses EXTRACT (returns a number directly). False means it uses
   * TO_CHAR and needs TO_NUMBER wrapping.
   */
