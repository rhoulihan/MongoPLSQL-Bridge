...
public final class StageParserRegistry {
  ...
  private void registerBuiltInParsers() {
    ...
    register(
        "$unionWith", value -> new UnionWithStageParser(new PipelineParser(this)).parse(value));
    ...
    register(
        "$facet", value -> new FacetStageParser(new PipelineParser(this)).parse((Document) value));
  ...
  private SortStage parseSortStage(Object value) {
    ...
    for (Map.Entry<String, Object> entry : sortDoc.entrySet()) {
      ...
      sortFields.add(
          new SortField(FieldPathExpression.of(fieldName), SortDirection.fromMongo(direction)));
    }
