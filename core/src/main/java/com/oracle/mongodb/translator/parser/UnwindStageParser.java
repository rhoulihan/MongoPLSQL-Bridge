...
public final class UnwindStageParser {
    ...
    private UnwindStage parseDocument(Document doc) {
        ...
        if (pathObj == null) {
            throw new IllegalArgumentException("$unwind requires 'path' field when using document format");
        }
        ...
        if (indexObj != null) {
            if (!(indexObj instanceof String)) {
                throw new IllegalArgumentException(
                    "$unwind 'includeArrayIndex' must be a string, got: " + indexObj.getClass().getSimpleName());
            }
