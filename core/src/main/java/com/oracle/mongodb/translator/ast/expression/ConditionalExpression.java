...
/**
 * Represents a conditional expression like $cond or $ifNull.
 * Translates to Oracle's CASE WHEN or NVL.
 *
 * <p>Examples:
 * <ul>
 *   <li>{@code {$cond: [condition, thenValue, elseValue]}} becomes
 *       {@code CASE WHEN condition THEN thenValue ELSE elseValue END}</li>
 *   <li>{@code {$ifNull: [field, default]}} becomes {@code NVL(field, default)}</li>
 * </ul>
 */
public final class ConditionalExpression implements Expression {
    ...
    private ConditionalExpression(ConditionalType type, Expression condition,
     ...
     */
    public static ConditionalExpression cond(Expression condition, Expression thenExpr, Expression elseExpr) {
        return new ConditionalExpression(ConditionalType.COND,
    ...
    public void render(SqlGenerationContext ctx) {
        switch (type) {
            case COND -> renderCond(ctx);
            case IF_NULL -> renderIfNull(ctx);
        }
    }
    ...
    public enum ConditionalType {
        ...
        public static ConditionalType fromMongo(String mongoOp) {
            return switch (mongoOp) {
                case "$cond" -> COND;
                case "$ifNull" -> IF_NULL;
                default -> throw new IllegalArgumentException("Unknown conditional operator: " + mongoOp);
            };
        }
