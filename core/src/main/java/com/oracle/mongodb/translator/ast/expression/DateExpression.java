...
/**
 * Represents a date expression that extracts date components. Translates MongoDB date operators to
 * Oracle EXTRACT or TO_CHAR functions.
 *
 * <p>Examples:
 *
 * <ul>
 *   <li>{@code {$year: "$createdAt"}} becomes {@code EXTRACT(YEAR FROM
 *       TO_TIMESTAMP(JSON_VALUE(data, '$.createdAt'), ...))}
 *   <li>{@code {$month: "$date"}} becomes {@code EXTRACT(MONTH FROM TO_TIMESTAMP(JSON_VALUE(data,
 *       '$.date'), ...))}
 *   <li>{@code {$dayOfWeek: "$date"}} becomes {@code TO_NUMBER(TO_CHAR(TO_TIMESTAMP(...), 'D'))}
 * </ul>
 *
