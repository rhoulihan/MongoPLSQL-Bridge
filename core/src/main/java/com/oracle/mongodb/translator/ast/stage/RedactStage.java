 ...
 *
 * <p>The $redact stage uses a conditional expression to determine if a document or embedded
 * document should be included in the output. The expression must evaluate to one of three system
 * variables:
 *
 * <ul>
 *   <li>$$DESCEND - keep this document and descend into embedded documents
 *   <li>$$PRUNE - exclude this document/subdocument
 *   <li>$$KEEP - keep this document but do not descend
 * </ul>
 *
 * <p>In Oracle SQL, this is translated to conditional filtering using CASE WHEN. Note that full
 * recursive document-level redaction is complex to implement in SQL, so this provides a simplified
 * implementation for document-level filtering.
 *
 * <p>Example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>translates to filtering based on the condition.
 */
