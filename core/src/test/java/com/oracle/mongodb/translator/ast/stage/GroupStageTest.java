...
class GroupStageTest {
  ...
  void shouldRenderGroupAllWithCount() {
    ...
    assertThat(context.toSql()).isEqualTo("SELECT COUNT(*) AS count");
  }
  ...
  void shouldRenderGroupAllWithMinMax() {
    ...
    accumulators.put(
        "minScore",
        AccumulatorExpression.min(FieldPathExpression.of("score", JsonReturnType.NUMBER)));
    accumulators.put(
        "maxScore",
        AccumulatorExpression.max(FieldPathExpression.of("score", JsonReturnType.NUMBER)));
